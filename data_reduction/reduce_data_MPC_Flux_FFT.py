...
for d in directories[mpi_rank::mpi_size]:
    ...
    # average work #
    ################
    # no difference for this part compared to reduce_data_MPC.py
    # write averaged data
    ...
    # FFT work #
    ############
    # this differs from reduce_data_MPC.py
    outputfilename = "reduced_data_fft_power_flux_"+d
    already_done = len(glob.glob(outputfilename))>0
    if do_fft and not already_done:
        ...
        fft = eds.fourier("he"+energyGroup, \
                field_N="ee"+energyGroup,nproc=nproc)
        fout["k"] = get_kmid(fft)
        cleft, cright, ileft, iright, kmid = fft_coefficients(fft)
        FZ00_FFT = fft_power(fft, cleft, cright, ileft, iright, kmid)
        fft = eds.fourier("ha"+energyGroup, \
                field_N="ea"+energyGroup,nproc=nproc)
        FZ11_FFT = fft_power(fft, cleft, cright, ileft, iright, kmid)
        fft = eds.fourier("hr"+energyGroup,field_Ph="hp"+energyGroup, \
                field_N="er"+energyGroup,field_Nph="ep"+energyGroup,nproc=nproc)
        FZ01_FFT = fft_power(fft, cleft, cright, ileft, iright, kmid)
