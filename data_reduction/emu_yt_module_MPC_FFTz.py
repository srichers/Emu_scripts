...
#                self.ds.add_field(("gas", fname_norm), function=_norm_derived_f, sampling_type="local", units="auto", dimensions=dimensions.dimensionless)
    ...
    def fourier(self, field_Mag, field_Ph=None, field_N=None, field_Nph=None, nproc=None):

        if field_N:
            if field_Ph:
                if field_Nph:
                    FT = np.squeeze(self.cg[field_Mag][:,:,:].d*self.cg[field_N][:,:,:]* \
                            (np.cos(self.cg[field_Ph][:,:,:].d + self.cg[field_Nph][:,:,:].d) \
                            + 1j*np.sin(self.cg[field_Ph][:,:,:].d + self.cg[field_Nph][:,:,:].d)))
                else:
                    print("Energy density phase not provided; aborting")
                    return FourierData(self.ds.current_time, 0, 0, 0, 0, 0)
            else:
                FT = np.squeeze(self.cg[field_Mag][:,:,:].d*self.cg[field_N][:,:,:].d)
        else:
            if field_Ph:
                FT = np.squeeze(self.cg[field_Mag][:,:,:].d*np.cos(self.cg[field_Ph][:,:,:].d) \
                        + 1j * self.cg[field_Mag][:,:,:].d*np.sin(self.cg[field_Ph][:,:,:].d))
            else:
                FT = np.squeeze(self.cg[field_Mag][:,:,:].d)
