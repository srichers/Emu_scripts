...
class Angles:
    ...
    def angle_at_point(self, location):
        ...
        SP = sft.SpinParams(t_sim = self.it, 
                        ...
                        merger_data_loc = self.merger_data_file,
                        gradient_filename = self.gradient_data_file,
                        location = location,
                        p_abs = self.p_abs)
        ...
        if SP.resonant_theta(phi=0) == None:
            ...
            if   self.method == 'solid angle':
                return SP.solidAngle(separate_ranges=self.separate_ranges, **self.kwargs)
            elif self.method == 'adiabatic angle': 
                return SP. findAdiabaticRegions(**self.kwargs)
            elif self.method == 'resonant angle':
                return SP.findResonantRegions(**self.kwargs)['total_width']
