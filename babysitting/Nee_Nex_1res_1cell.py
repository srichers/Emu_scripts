...
#Beam
n_nue0 = 1.1*4.89e+32     # 1/ccm
n_nux0 = 4.89e+32/11.0 #1/ccm
mfact = 1.e-32
...
def plotdata(filename,nutype,a,b):
    ...
    t=np.array(Data["t"])*1e9
    if nu_type == 1:
        N=np.sqrt(np.array(Data["Nbar_real"])[:,a,b]**2 + np.array(Data["Nbar_imag"])[:,a,b]**2)
    else:
        N=np.sqrt(np.array(Data["N_real"])[:,a,b]**2 + np.array(Data["N_imag"])[:,a,b]**2)
    Data.close()
    return t, N
...
if nu_type == 1:
    axes[0].set_ylabel(mfactstr + r'$\langle \overline{N}_{ee}\rangle\,({\rm cm}^{-3})$')
    axes[1].set_ylabel(mfactstr + r'$|\overline{N}_{ex}|\,({\rm cm}^{-3})$')
else:
...
# plot data #
#############
filename_bang = "reduced_data_8_cell_4.h5"
#fid
box_length = 8.0
n_grid = 128
#beam
...
namestr = "Nee_Nex_1res_" + nu_type_str + ".pdf"
