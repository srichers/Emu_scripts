...
def plotdata(filename,a,b):
    ...
    if nu_type == 1:
        N=np.array(avgData["Fbar_avg_mag"])[:,flux_ind,a,b]
    else:
        N=np.array(avgData["F_avg_mag"])[:,flux_ind,a,b]
    avgData.close()
    return t, N
...
if nu_type == 1:
    ax.set_ylabel(r"$\langle |\overline{F}^{z}_{ex}|\rangle /\mathrm{Tr}(\overline{N})$")
else:
    ax.set_ylabel(r"$\langle |F^{z}_{ex}|\rangle /\mathrm{Tr}(N)$")
...
namestr = "Fex_z_test_comp_" + nu_type_str + ".pdf"
