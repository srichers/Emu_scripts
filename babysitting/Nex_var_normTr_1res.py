...
if nu_type == 1:
    ...
    axes[1].set_ylabel(r'$\sigma^2(|\overline{N}_{ex}|)/{\rm Tr}^2[\overline{N}]$')
else:
    axes[0].set_ylabel(r'$\langle|N_{ex}|\rangle/{\rm Tr}[N]$')
    axes[1].set_ylabel(r'$\sigma^2(|N_{ex}|)/{\rm Tr}^2[N]$')
...
t,Nex = plotdata(filename_bang,nu_type,0,1)
tmax = t[np.argmax(Nex)]
t_var,Nex_var = plotdata_var(filename_bang,nu_type,0,1)
#t,Nex = plotdata(filename_bang,nu_type,0,0)
#t_var,Nex_var = plotdata_var(filename_bang,nu_type,0,0)
print('tmax = ', tmax)
axes[0].semilogy(t-tmax, Nex, 'r-', label='2f')
#axes[0].plot(t-tmax, Nex, 'r-', label='2f')
axes[1].semilogy(t-tmax, Nex_var, 'r-', label='2f')
...
namestr = "Nex_var_1res_normTr_" + nu_type_str + ".pdf"
#namestr = "Nee_var_1res_normTr_" + nu_type_str + ".pdf"
