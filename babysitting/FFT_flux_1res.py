...
# read averaged data #
######################
def plotdata(filename_FFT, filename_avg, filename_norm, t_in):
    ...
    normData = h5py.File(filename_norm,"r")
    Nee=np.array(normData["N00_FFT"])
    Nxx=np.array(normData["N11_FFT"])
    normData.close()
...
ax.set_ylabel(r"$\mathcal{D}_F(k)$")
#one time only:
...
filename_bang   = "reduced_data_flux_fft_power.h5"
filename_bang_avg   = "reduced_data.h5"
#used for proper normalization of FFT:
filename_bang_norm   = "reduced_data_fft_power.h5"

k3,N3 = plotdata(filename_bang,filename_bang_avg,filename_bang_norm,tplot)
...
plt.savefig("FZex_FFT_1res.pdf", bbox_inches="tight")
