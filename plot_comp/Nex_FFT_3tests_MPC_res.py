...
basedirs = ["/global/cfs/projectdirs/m3761/FLASH/Emu/",
            "/global/cfs/projectdirs/m3761/FLASH/Emu/",
            "/global/cfs/projectdirs/m3761/FLASH/FFI_3D/"]
savedir = "/global/cfs/projectdirs/m3761/FLASH/FFI_3D/3tests/MPC/emu_comp/"
simlist_fid = ["Fiducial_3D_2F", "Fiducial_3D_3F", "fid/MPC/d_pert/t5/xy_large/sim"]
simlist_90deg = ["90Degree_3D_2F", "90Degree_3D_3F", "90d/MPC/d_pert/t3/xy_large/sim"]
simlist_23 = ["TwoThirds_3D_2F", "TwoThirds_3D_3F", "2_3/MPC/d_pert/t3/xy_large/sim"]

test_titles = [r'${\rm Fiducial}$', r'${\rm 90Degree}$', r'${\rm TwoThirds}$']

itmax_inds = -np.ones([3,2], dtype=np.int8)
itmax_inds[1,1] = 118
...
def makeplot(ax, simlist, ind, ind_pow, ind_avg):
    ...
    print('Flash sim')
    k3,N3 = plotdata(filename_bang,filename_bang_avg,tplot,ind_pow[2],ind_avg[2],itmax=itmax_inds[ind,1])
    ...
    if ind==2:
        filename_bang   = basedirs[2]+simlist[2]+"/../res_b/reduced_data_fft_power.h5"
        filename_bang_avg   = basedirs[2]+simlist[2]+"/../res_b/reduced_data.h5"
        print('Flash res2')
        k3,N3 = plotdata(filename_bang,filename_bang_avg,tplot,ind_pow[2],ind_avg[2])
        ax.semilogy(k3, N3, 'r-', alpha=0.25, label=None)
        k_ind = np.argmax(N3)
...
axes[2].axvline(1.49, color='green', label=None)
    
axes[0].legend(loc='upper right', fontsize=28, frameon=False)
plt.savefig(savedir + "Nex_FFT_3tests_MPC_res.pdf", bbox_inches="tight")
