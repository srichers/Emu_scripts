...
#gathering all 2D pltfile directories
directory_inputs = sorted(glob.glob("/ocean/projects/phy200048p/shared/2D/*/"))
#looping through directory names and creating output plot directories corresponding to each input
for d in directory_inputs:
    ...
    #creating output sub-directory in plots directory
    directory_out = "/ocean/projects/phy200048p/shared/plots/"+directory_current
    #checking if the directory doesnt exist, or if it doesn't have any subplot files inside... if so, create the directory and do the subplot routine. If directory exists and has subplot files, skip it
    if os.path.exists(directory_out) == True and len(glob.glob(directory_out+"/*3x3subplot.png")) != 0:
        print("output directory already exists and contains 3x3 subplots")
        continue
    #If directory doesnt have subplot files in it, run the subplot routine
    else:
        print("output directory doesn't have 3x3 subplot files in it yet, running subplot script")
        #if the directory doesn't exist, make it first
        if not os.path.exists(directory_out):
            print("output directory does not exist, creating directory")
            os.makedirs(directory_out)
        #grab all pltfiles for creating subplots
        filenames = sorted(glob.glob(d+"plt*"))
        for filename in filenames:
            ...
            fig.savefig(directory_out+"/"+"{0}_3x3subplot.png".format(ds),dpi=300, bbox_inches="tight")
