...
def plotdata(filename_avg, d, a, b):
    ...
    #if filename_avg[-5:] == 'hk.h5':
    #    print(filename_avg, str(d), str(ff[0]/Num[0]*flash_ffact_conv))
    #else:
    #    print(filename_avg, str(d), str(ff[0]/Num[0]))
...
axes[0].set_ylabel(r"$\langle |f^{(x)}_{ee}|\rangle$")
axes[1].set_ylabel(r"$\langle |f^{(y)}_{ee}|\rangle$")
axes[2].set_ylabel(r"$\langle |f^{(z)}_{ee}|\rangle$")
...
# save pdf #
############
plt.savefig("ffact_3dirs_avgF_ee.pdf", bbox_inches="tight")
