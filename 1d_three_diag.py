...
def snapshot_plot(ad, axes, t):
    ...
    ax = axes[0]
    ax.plot(z, ad['boxlib',"N00_Re"   ]/trace, color="blue", label=r"${ee}$")
    ax.plot(z, ad['boxlib',"N11_Re"   ]/trace, color="red" , label=r"${\mu\mu}$")
    ax.plot(z, ad['boxlib',"N22_Re"   ]/trace, color="gold", label=r"${\tau\tau}$")
    ax.plot(z, ad['boxlib',"N00_Rebar"]/tracebar, color="blue",
             linestyle="--", label=r"$\bar{ee}$")
    ax.plot(z, ad['boxlib',"N11_Rebar"]/tracebar, color="red",
             linestyle="--", label=r"$\bar{\mu\mu}$")
    ax.plot(z, ad['boxlib',"N22_Rebar"]/tracebar, color="gold",
             linestyle="--", label=r"$\bar{\tau\tau}$")


    #ax = axes[1]
    #ax.plot(z, (ad['boxlib',"Fz00_Re"   ])/trace, color="blue", label=r"$|f_{ee}|$")
    #ax.plot(z, (ad['boxlib',"Fz11_Re"   ])/trace, color="red" , label=r"$|f_{\mu\mu}|$")
    #ax.plot(z, (ad['boxlib',"Fz22_Re"   ])/trace, color="gold", label=r"$|f_{\tau\tau}|$")
    #ax.plot(z, (ad['boxlib',"Fz00_Rebar"])/tracebar, color="blue",
    #         linestyle="--", label=r"$\bar{f}_{ee}$")
    #ax.plot(z, (ad['boxlib',"Fz11_Rebar"])/tracebar, color="red",
    #         linestyle="--", label=r"$\bar{f}_{\mu\mu}$")
    #ax.plot(z, (ad['boxlib',"Fz22_Rebar"])/tracebar, color="gold",
    #         linestyle="--", label=r"$\bar{f}_{\tau\tau}$")

    ax = axes[1]
    fac=1e6
    ax.plot(z, fac*(ad['boxlib',"Fx00_Re"   ])/trace, color="blue", label=r"${ee}$")
    ax.plot(z, fac*(ad['boxlib',"Fx11_Re"   ])/trace, color="red" , label=r"${\mu\mu}$")
    ax.plot(z, fac*(ad['boxlib',"Fx22_Re"   ])/trace, color="gold", label=r"${\tau\tau}$")
    ax.plot(z, fac*(ad['boxlib',"Fx00_Rebar"])/tracebar, color="blue",
             linestyle="--", label=r"$\bar{ee}$")
    ax.plot(z, fac*(ad['boxlib',"Fx11_Rebar"])/tracebar, color="red",
             linestyle="--", label=r"$\bar{\mu\mu}$")
    ax.plot(z, fac*(ad['boxlib',"Fx22_Rebar"])/tracebar, color="gold",
             linestyle="--", label=r"$\bar{\tau\tau}$")
...
for i,ax in zip(range(3),axes[0,:]):
    ...
    ax.set_xticklabels([])
    ax.set_ylim(-.05,1.25)
    ax.text(8,1.07, "t=%0.2f ns"%(t[i]*1e9), ha="center", va="center")
...
for ax in axes[1,:]:
    #ax.set_yscale("log")
    ax.set_ylim(-5,4.99) # 1e-18 for first panel
...
axes[0,0].legend(frameon=False,loc='lower center',ncol=2)
...
plt.savefig("1d_three_diag.pdf",bbox_inches="tight")
